      ...
      <div class="grid grid-cols-1 md:grid-cols-2">
              ...
              @if ($room->images->isNotEmpty())
                  <div class="carousel-container absolute top-0 left-0 w-full h-full">
                    @foreach ($room->images as $image)
                      <div class="carousel-slide absolute top-0 left-0 w-full h-full transition-opacity duration-500 opacity-0" style="opacity: 1;">
                        <img src="{{ $image->path }}" alt="{{ $image->alt_text }}" class="object-cover w-full h-full">
                      </div>
                    @endforeach
                      <button class="absolute top-1/2 left-2 sm:left-4 transform -translate-y-1/2  bg-opacity-50 text-white rounded-full w-8 h-8 flex justify-center items-center focus:outline-none" onclick="prevSlide()">
                          <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
                      </button>
                      <button class="absolute top-1/2 right-2 sm:right-4 transform -translate-y-1/2 bg-opacity-50 text-white rounded-full w-8 h-8 flex justify-center items-center focus:outline-none" onclick="nextSlide()">
                          <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
                      </button>
                      <div class="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex space-x-2">
                          @foreach ($room->images as $index => $image)
                              <button class="w-2 h-2 sm:w-3 sm:h-3 rounded-full bg-gray-300 focus:outline-none" onclick="goToSlide({{ $index }})"></button>
                          @endforeach
                      </div>
                  </div>
              @else
          ...
          <div class="p-6">
              ...
              <div class="flex justify-between mb-4">
                  <a href="{{ route('rooms.index') }}" class="bg-violet-500 text-white py-2 px-4 rounded-md">Kembali</a>
                  <div class="bg-purple-400 text-white py-2 px-4 rounded-md">Harga: Rp {{ number_format($room->harga_room, 0, ',', '.') }}</div>
              </div>

              <div class="flex justify-between mb-4">
                  @if (isset($room->rating))
                      <button class="bg-yellow-300 text-gray-800 py-2 px-4 rounded-md flex items-center">
                          <svg class="h-5 w-5 text-yellow-500 mr-1" viewBox="0 0 20 20" fill="currentColor"><path d="M9.049 2.928c.374-1.136 1.607-1.136 1.981 0l2.692 8.156c.265.808.998 1.392 1.79 1.392h8.418c1.03 0 1.53.667.997 1.22l-6.22 5.93c-.26.248-.356.577-.268.893l2.692 8.157c.374 1.136-.088 2.272-1.981 2.272H3.099c-1.893 0-2.355-1.136-1.982-2.272l2.692-8.157c.088-.316-.008-.645-.269-.893l-6.22-5.93c-.533-.553-.032-1.22.998-1.22h8.418c.792 0 1.525-.584 1.79-1.392l2.692-8.156z" /></svg>
                          {{ $room->rating }}
                      </button>
                  @else
              ...
              <div class="grid grid-cols-5 gap-2 mb-4">
                  @for ($i = 1; $i <= 20; $i++)
                      ...
                      @endphp
                      <button class="{{ $tersedia }} text-white py-2 px-3 rounded-md text-xs">A{{ $i }}</button>
                  @endfor
              </div>
