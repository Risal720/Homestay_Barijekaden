    ...
    <body class="bg-gray-100 p-8">
        <div class="container mx-auto bg-white p-6 rounded-lg shadow-xl">
            ...
            {{-- Tombol Tambah Pesanan (jika ada form create pesanan) --}}
            <div class="mb-6">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full bg-white rounded-lg shadow-md">
                    ...
                    <tbody class="text-gray-600 text-sm font-light">
                        {{-- Perulangan untuk menampilkan setiap pesanan --}}
                        @forelse($orders as $order)
                        <tr class="border-b border-gray-200 hover:bg-gray-100">
                            <td class="py-3 px-6 text-left whitespace-nowrap">{{ $order->id }}</td>
                            <td class="py-3 px-6 text-left">{{ $order->customer_name ?? 'N/A' }}</td> {{-- Sesuaikan dengan kolom Anda --}}
                            <td class="py-3 px-6 text-left">Rp {{ number_format($order->total_price ?? 0, 0, ',', '.') }}</td> {{-- Sesuaikan dengan kolom Anda --}}
                            <td class="py-3 px-6 text-left">{{ $order->status ?? 'N/A' }}</td> {{-- Sesuaikan dengan kolom Anda --}}
                            <td class="py-3 px-6 text-center">
                                <div class="flex item-center justify-center">
                                    ...
                                    <a href="{{ route('admin.orders.edit', $order->id) }}" class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                        {{-- Icon untuk Edit --}}
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        @empty
                        <tr>
                            <td colspan="5" class="py-3 px-6 text-center text-gray-500">Belum ada pesanan yang ditemukan.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
